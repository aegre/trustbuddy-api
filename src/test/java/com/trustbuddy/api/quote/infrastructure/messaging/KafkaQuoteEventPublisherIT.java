...
class KafkaQuoteEventPublisherIT extends FullInfrastructureTestcontainers {
		...
		@Autowired private KafkaQuoteEventPublisher quoteEventPublisher;
		...
		void givenSubmittedQuote_whenPublishQuoteSubmitted_thenEventIsPublishedToTopic() {
				...
				try (KafkaConsumer<String, QuoteSubmittedEvent> consumer = createConsumer()) {
						...
						assertThat(event.premium())
										.isEqualByComparingTo(submitted.getEstimatedMonthlyPremium());
		...
		private KafkaConsumer<String, QuoteSubmittedEvent> createConsumer() {
				...
				properties.put(
								...
								"com.trustbuddy.api.quote.infrastructure.messaging");
