...
public class QuotePersistenceAdapter implements QuoteRepositoryPort {
		...
		public QuotePersistenceAdapter(
						QuoteJpaRepository jpaRepository, QuotePersistenceMapper mapper) {
		...
		public Quote save(Quote quote) {
				QuoteEntity entity = jpaRepository.findById(quote.getId()).orElseGet(QuoteEntity::new);
		...
		public List<Quote> findStaleDrafts(Instant updatedBefore) {
				return jpaRepository
								.findByStatusAndUpdatedAtBefore(QuoteStatus.DRAFT, updatedBefore)
								.stream()
								.map(mapper::toDomain)
								.toList();
		}
